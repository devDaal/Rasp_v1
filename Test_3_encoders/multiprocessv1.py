...
def gpio_listener(process_id, A, B, C, event, queue):
    """
    ...
    while True:
        ...
        pinB = gp.input(B)
        pinA = gp.input(A)
        if Encoder.updated_reference_mode:                        #¿Funciona decir el nombre de la clase encoder? o ¿Debe ser el nombre de la instancia app?
            pinC = gp.input(C)
            if lastC != pinC:
                refmark = True
                Encoder.ref_mark_1_lbl['bg'] = 'green'#ID de reference mark       ¿Cómo poner para que se ponga en verde el reference mark de cada encoder? ¿Se puede poner aqui 
            lastC = pinC                        #              |                  un id de reference mark para enviarlo abajo junto con el process id y el contador al queue?
        if lastB != pinB or lastA != pinA:      #              |_____ 
        ...
        lastB = pinB                            #                   |
        lastA = pinA                            #                   |
        counter = positive - negative           #                   v
        queue.put((process_id, counter,refmark))        #        Envía el valor al Queue
...
class Encoder(Frame):
    ...
    def update_ui(self):
        ...
        for idx, queue in enumerate(self.queues):
            while not queue.empty():
                process_id, counter, refmark = queue.get()
                if process_id == "x":
                    self.distance_lbl_encoder_1.config(text=f"{counter}")
                    if refmark == True:
                        self.ref_mark_1_lbl['bg'] = 'green'
                elif process_id == "y":
                    self.distance_lbl_encoder_2.config(text=f"{counter}")
                    if refmark == True:
                        self.ref_mark_2_lbl['bg'] = 'green'
                elif process_id == "z":
                    self.distance_lbl_encoder_3.config(text=f"{counter}")
                    if refmark == True:
                        self.ref_mark_3_lbl['bg'] = 'green'
    ...
    def configurar_procesos(self):
        ...
        Position1 = [("Position 1","x"),
                     ("Resolution",500),
                     ("A", 35),
                     ("B", 36),
                     ("C", 37)]
        
        Position2 = [("Position 2","y"),
                     ("Resolution",1000),
                     ("A", 29),
                     ("B", 31),
                     ("C", 33)]
        ...
        encoder_axis = [
            ("Position 1", Position1),  # (Nombre, Intervalo de lectura)
            ("Position 2", Position2)]
        
        
        for juego, resolution in encoder_axis:
            event = Event()  # Evento para controlar pausa/reanudación
            queue = Queue()  # Cola para recibir datos del proceso
            process = Process(target=gpio_listener, args=(juego,resolution ,event, queue))
            ...
            """     
            Position1 = ["x", 1000, 10,11,12]
            Position2 = ["y", 5000, 13,14,15]
            Position = [Position1,Position2]

            def ejemplo(axis,resolution,A,B,C):
                eje = axis
                res = resolution
                print(eje,res)
                print(A)
            ...
            for i in Position:
                position = i
                ejemplo(position[0],position[1],position[2],position[3],position[4])
                
            Esto funciona para iterar la funcion de forma independiente y poder acceder
            a los argumentos diferentes en cada iteracion, pero, todavía necesito ver
            como crear la lista a partir del numero de encoders seleccionado y como
            cambiar la resolucion de cada uno de forma independiente
